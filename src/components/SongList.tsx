...
interface SongListProps {
  ...
  title: string;
  emptyLabel?: string;
  /** 빈 상태 헤더 오른쪽에 표시되는 액션 슬롯 (예: 제보하기 버튼). */
    ...
    <section className="mb-10">
      {hasSongs ? (
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          aria-expanded={open}
          className="w-full flex items-center justify-between gap-3 mb-4 rounded-2xl p-4 transition-colors bg-surface-container-low hover:bg-primary-fixed cursor-pointer group"
        >
          {headerInner}
          <ChevronDown
        ...
        <div className="flex items-center justify-between gap-3 mb-4 rounded-2xl p-4 bg-surface-container-low/50">
          {headerInner}
          {emptyAction}
        </div>
      )}
